...
inline napi_status NewString(napi_env env,
                             const CCharType* str,
                             size_t length,
                             napi_value* result,
...
inline napi_status NewExternalString(napi_env env,
                                     CharType* str,
                                     size_t length,
  ...
  if (status == napi_ok && copied != nullptr) *copied = false;
#endif
  return status;
}
...
class ExternalStringResource final
...
inline napi_status Unwrap(napi_env env,
  ...
  if (action == KeepWrap) CHECK_ARG(env, result);
  ...
  if (result) *result = reference->Data();
 ...
 public:
  ...
  void Args(napi_value* buffer, size_t buffer_length) {
    ...
    if (i < buffer_length) {
      ...
      for (; i < buffer_length; ++i) buffer[i] = undefined;
    }
  }
  ...
  void InvokeCallback() {
    ...
    env->CallIntoModule([&](napi_env env) { result = cb(env, cbinfo_wrapper); },
                        [&](napi_env env, v8::Local<v8::Value> value) {
                          ...
                          if (env->terminatedOrTerminating()) return;
...
inline napi_status Wrap(napi_env env,
  ...
  if (result != nullptr) {
    ...
    reference = v8impl::ReferenceWithFinalizer::New(
        env, obj, 0, v8impl::ReferenceOwnership::kUserland,
        finalize_cb, native_object, finalize_hint);
    *result = reinterpret_cast<napi_ref>(reference);
  } else if (finalize_cb != nullptr) {
    reference = v8impl::ReferenceWithFinalizer::New(
        env, obj, 0, v8impl::ReferenceOwnership::kRuntime,
        finalize_cb, native_object, finalize_hint);
  } else {
...
inline napi_status set_error_code(napi_env env,

...
int OnHeaderValue(llhttp_t* p, const char* at, size_t len) {
  ...
  if (st->current_header == "upgrade") {
    for (auto& c : v) if (c >= 'A' && c <= 'Z') c |= 0x20;
    if (v.find("websocket") != std::string::npos) st->websocket = true;
  } else if (st->current_header == "connection") {
    for (auto& c : v) if (c >= 'A' && c <= 'Z') c |= 0x20;
    if (v.find("upgrade") != std::string::npos) st->upgrade = true;
...
bool InspectorServer::ServeConnection(int sock) {
  ...
  while (!state.complete && !stop_.load()) {
    ...
    if (err != HPE_OK && err != HPE_PAUSED_UPGRADE) return false;
  }
  ...
  if (!SendHttp(sock, resp)) return false;
...
void InspectorServer::ClientReadLoop(int sock, std::string leftover) {
  WsDecoder dec;
  if (!leftover.empty()) dec.Feed(leftover.data(), leftover.size());
  char tmp[4096];
  WsFrame frame;
  ...
  while (!stop_.load()) {
    ...
    while (dec.TryDecode(&frame) == 1) {
      if (frame.op == WsOp::kClose) return;
      ...
      if (frame.op == WsOp::kText || frame.op == WsOp::kBinary ||
          frame.op == WsOp::kContinuation) {
        assembled += frame.payload;
        if (frame.fin) {
          if (on_message_) on_message_(assembled);
          assembled.clear();
        }
      }
    }
...
void InspectorServer::ClientWriteLoop(int sock) {
  while (!stop_.load()) {
    std::string msg;
    {
      ...
      if (stop_.load() && out_q_.empty()) return;

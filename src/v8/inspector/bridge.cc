...
std::string Sv2str(const StringView& v) {
  if (v.is8Bit())
    return std::string(reinterpret_cast<const char*>(v.characters8()),
                       v.length());
...
InspectorBridge::InspectorBridge(v8::Isolate* iso,
...
InspectorBridge::~InspectorBridge() {
  Stop();
  if (inspector_) inspector_->contextDestroyed(context_.Get(isolate_));
}

bool InspectorBridge::Start(int port) {
  server_.SetOnMessage([this](const std::string& msg) {
    DispatchProtocolMessage(msg);
  });
  if (!server_.Start(port, context_name_)) return false;
  ...
  session_ = inspector_->connectShared(
      /*contextGroupId=*/1, channel_.get(), Sv(std::string()),
...
v8::Local<v8::Context> InspectorBridge::ensureDefaultContextInGroup(int) {

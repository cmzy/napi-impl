...
napi_v8::inspector::InspectorBridge* GetOrCreate(napi_env env,
                                                 const char* name) {
  std::lock_guard<std::mutex> g(g_mu);
  auto it = g_bridges.find(env);
  if (it != g_bridges.end()) return it->second;
  auto* br = new napi_v8::inspector::InspectorBridge(
      env->isolate, env->context(),
...
extern "C" napi_status NAPI_CDECL
napi_v8_inspector_start(napi_env env, int port, const char* context_name) {
  if (env == nullptr || port <= 0 || port > 65535) return napi_invalid_arg;
...
napi_v8_inspector_wait_for_connection(napi_env env) {
  ...
  std::lock_guard<std::mutex> g(g_mu);
  auto it = g_bridges.find(env);
  if (it == g_bridges.end()) return napi_invalid_arg;
...
extern "C" napi_status NAPI_CDECL napi_v8_inspector_stop(napi_env env) {
  ...
  napi_v8::inspector::InspectorBridge* br = nullptr;
  {
    std::lock_guard<std::mutex> g(g_mu);
    auto it = g_bridges.find(env);
    if (it == g_bridges.end()) return napi_ok;
    br = it->second;
    g_bridges.erase(it);
  }
  br->Stop();

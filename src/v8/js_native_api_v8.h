...
struct napi_env__ {
    explicit napi_env__(v8::Local<v8::Context> context, int32_t module_api_version)
        // V8 14.x removed v8::Context::GetIsolate(); we read the active
        // isolate via Isolate::GetCurrent() (callers must enter the isolate
        // before constructing napi_env__).
        :
        isolate(v8::Isolate::GetCurrent()),
        context_persistent(isolate, context), module_api_version(module_api_version) {
        napi_clear_last_error(this);
    }

    inline v8::Local<v8::Context> context() const { return v8impl::PersistentToLocal::Strong(context_persistent); }
    ...
    inline bool terminatedOrTerminating() { return this->isolate->IsExecutionTerminating() || !can_call_into_js(); }
    ...
    // into JavaScript.
    virtual void EnqueueFinalizer(v8impl::RefTracker *finalizer) { pending_finalizers.emplace(finalizer); }
    ...
    void CheckGCAccess() {
        if (module_api_version == NAPI_VERSION_EXPERIMENTAL && in_gc_finalizer) {
            v8impl::OnFatalError(nullptr, "Finalizer is calling a function that may affect GC state.\n"
...
inline napi_status napi_set_last_error(node_api_basic_env basic_env, napi_status error_code,
...
#define NAPI_PREAMBLE(env)                                                                                             \
    ...
    RETURN_STATUS_IF_FALSE(                                                                                            \
            ...
            (env->module_api_version == NAPI_VERSION_EXPERIMENTAL ? napi_cannot_run_js : napi_pending_exception));     \
...
namespace v8impl {
    ...
    public:
        Finalizer(napi_env env, napi_finalize finalize_callback, void *finalize_data, void *finalize_hint) :
            env_(env), finalize_callback_(finalize_callback), finalize_data_(finalize_data),
    ...
    public:
        static TrackedFinalizer *New(napi_env env, napi_finalize finalize_callback, void *finalize_data,
                                     void *finalize_hint);
    ...
    public:
        static Reference *New(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount,
    ...
    public:
        static ReferenceWithData *New(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount,
                                      ReferenceOwnership ownership, void *data);

        void *Data() override { return data_; }

    private:
        ReferenceWithData(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount,
                          ReferenceOwnership ownership, void *data);
    ...
    public:
        static ReferenceWithFinalizer *New(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount,
    ...
    private:
        ReferenceWithFinalizer(napi_env env, v8::Local<v8::Value> value, uint32_t initial_refcount,
                               ReferenceOwnership ownership, napi_finalize finalize_callback, void *finalize_data,
                               void *finalize_hint);

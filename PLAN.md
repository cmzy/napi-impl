...
- [~] **M7.2 js-native-api 套件（45/50 通过，零崩溃，macOS + Linux x86_64）**：
  ...
  - 剩余 5 个失败(均为预期引擎语义差异,行为正确仅文案/策略不同,不修):(a) `test_constructor`/`6_object_wrap`/`test_object`/`test_properties`(只读/只-getter/不可扩展赋值的 `TypeError` **错误信息文案**与 V8 不同——抛错本身正确,源于 Hermes VM 的错误字符串);(b) `test_finalizer/test_fatal_finalize`(需 Node 的"finalizer 调 JS 即 fatal"语义 + spawnSync abort 检测,与我们 v9 deferred 策略 + runner 不符)。`testEnvCleanup`(patch 0005 + 完整 teardown)与 `test_array`(patch 0006)已转绿。
- [~] **M7.3 跨平台打包**：

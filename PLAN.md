...
- [~] **M7.2 js-native-api 套件（40/50 通过，零崩溃，Linux x86_64）**：
  ...
  - 剩余 10 个失败两类:(a) **引擎语义差异**(预期,行为正确仅文案/上限不同,不修):`test_array`(Hermes 拒绝 4G 元素数组)、`test_constructor`/`6_object_wrap`/`test_object`/`test_properties`(只读/只-getter/不可扩展赋值的 `TypeError` **错误信息文案**与 V8 不同——抛错本身正确);(b) **finalizer 语义 / teardown**:`test_finalizer/test_fatal_finalize`、`test_exception/testFinalizerException`、`test_reference/test_finalizer`、`6_object_wrap` 的 dangling-ref finalizer、`testEnvCleanup`(runner 跳 teardown 取舍)。
- [~] **M7.3 跨平台打包**：

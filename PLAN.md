...
- [~] **M7.2 js-native-api 套件（33/45 通过，零崩溃，Linux x86_64）**：
  ...
  - 已知 harness 限制(引擎无关,V8 路径同样存在):`test_object`/`test_reference` 等单目录多 `.c`(各自 `NAPI_MODULE`)被 `build_one` 合并为一个 `.so` → `napi_register_module_v1` 重定义,2 个目录 build 失败。

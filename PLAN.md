...
- [~] **M7.2 js-native-api 套件（45/50 通过，零崩溃，macOS + Linux x86_64）**：
  ...
  - **teardown UAF 修复(patch 0005)**:`napi_destroy_runtime` 的 deferred-finalizer-task UAF 已修(`EmbedTaskRunner` teardown 内联)。残留的 Hermes UAF 也已根因定位并修复——`NodeApiFinalizerHolder::invokeAllFinalizers()` 的 `while(!finalizers_.empty())` 循环依赖 `invokeFinalizerFromGC()` 把元素从集合摘掉,但它只经 ref 自身的 `finalizerHolder_` backlink 解链;**external buffer** 的 finalizer(无 JS 值→backlink 为 null,`~NodeApiExternalBuffer` 经 `addFinalizer` 加进临时 holder 时不设 backlink)摘不掉,且 `invokeFinalizerFromGC` 会 `delete` 该 ref(kRuntime 所有权)→ 下一圈对已释放指针调虚函数(null vtable→SIGSEGV)。只在 external arraybuffer 被 GC 回收(我们 runner 强制 `gc()`)后再 teardown 时触发;纯 teardown 走另一条正确路径故不崩。patch 0005 在循环里**先 erase 再 invoke**,使循环不依赖 backlink 总能推进。修复后 runner 默认改为**完整 teardown**(`test/runner.cc`,非 V8 路径),`testEnvCleanup` 与 `test_typedarray` 双双转绿,43→44/50。**应上报 microsoft/hermes-windows**。
  ...
  - **test_array 修复(patch 0006)**:经实测 Hermes 引擎完全支持稀疏 `new Array(2^32-1)`(`OK length=4294967295`),证明并非引擎上限,而是 node-api 的 `napi_create_array_with_length` 把 `capacity=length` 传给 `JSArray::create`→ 真分配 `length` 个元素的 ArrayStorage→`ArrayStorage.cpp` 抛 RangeError。Hermes 自己的 `Array(n)` 构造器是**建空(capacity 0)+ `setLengthProperty`**、从不预分配。patch 0006 照搬之 → `test_array` 转绿,44→45/50。**应上报 microsoft/hermes-windows**。
  - **ASan 排查 + bigint use-after-scope 修复(patch 0007)**:用 Hermes 自带的 `HERMES_ENABLE_ADDRESS_SANITIZER=ON` 把引擎 + 我们的 lib/runner 全量编译,跑完整套件。抓到一个**真实 UB**:`napi_create_bigint_words` 处理负数(signBit)时,把 2's-complement 写进**`if` 块内的** `llvh::SmallVector<uint64_t,16> negativeValue`,然后 `words = negativeValue.data()`;块结束后 `negativeValue` 析构,`words` 悬空,`fromBytes`→`initWithBytes` 仍 memcpy 之 → **stack/heap-use-after-scope**(`BigIntSupport.cpp:138`)。非 ASan 下因栈窗口未被覆写而"侥幸正确",ASan 必抓。patch 0007 把 `negativeValue` 提到函数作用域,使其活过 `fromBytes`。修复后 ASan 套件**零 AddressSanitizer 报错**;非 ASan 仍 45/50(此 bug 本就侥幸通过,修的是 UB/安全性,pass 数不变)。**应上报 microsoft/hermes-windows**(node-api 层第三个内存 bug)。
  - **(我们 JSC 后端经 ASan 全清)**:同样用 ASan 编译 JSC 后端,4 个单测零报错——wrap/external/reference/scope/promise/buffer 生命周期内存干净。
  - 剩余 5 个失败(均为预期引擎语义差异,行为正确仅文案/策略不同,不修):(a) `test_constructor`/`6_object_wrap`/`test_object`/`test_properties`(只读/只-getter/不可扩展赋值的 `TypeError` **错误信息文案**与 V8 不同——抛错本身正确,源于 Hermes VM 的错误字符串);(b) `test_finalizer/test_fatal_finalize`(需 Node 的"finalizer 调 JS 即 fatal"语义 + spawnSync abort 检测,与我们 v9 deferred 策略 + runner 不符)。`testEnvCleanup`(patch 0005 + 完整 teardown)与 `test_array`(patch 0006)已转绿。(注:`testFinalizerException` 仅在 ASan 下因 GC/finalizer 时序被打乱而失败、无 ASan 报错;真实非-ASan 构建中通过,非 bug。)
- [~] **M7.3 跨平台打包**：
